...
in {
  options = {
    perSystem = mkPerSystemOption ({ pkgs, ... }:
      let inherit (lib) mkOption types genAttrs;
      in {
          ...
          type = with types;
            submodule {
              options = {
                ...
                plugins = mkOption {
                  type = (submodule {
                    options = let _types = [ "lazy" "eager" ];
                    in genAttrs _types (_:
                      mkOption {
                        type = listOf str;
  ...
  config = {
    perSystem = { config, lib, pkgs, system, ... }:
      let
        inherit (lib)
          getExe makeBinPath escapeShellArgs concatStringsSep
          concatMapStringsSep mkIf concatLists;
                ...
                paths = [
                  ...
                  (map grammarToPlugin
                    (pkgs.vimPlugins.nvim-treesitter.allGrammars
                      ++ (__attrValues (__lib.mkTreesitter cfg.extraParsers))))
                ];
              })
              parinfer-rust
        ...
        wrapperArgs = let binpath = makeBinPath cfg.bins;
        in escapeShellArgs NeovimConfig.wrapperArgs + " " + ''
          --suffix LUA_CPATH ";" "${
            concatMapStringsSep ";" pkgs.lua51Packages.getLuaCPath
            (with pkgs.lua51Packages; cfg.lua.extraPacks)
          }"'' + " " + ''
            --suffix LUA_PATH ";" "${
              concatMapStringsSep ";" pkgs.lua51Packages.getLuaPath
              (with pkgs.lua51Packages; cfg.lua.extraPacks)
            };${
              concatMapStringsSep ";" pkgs.luajitPackages.getLuaPath
              (with pkgs.luajitPackages; cfg.lua.jit)
            }"'' + " " + "--prefix PATH : ${binpath}" + " "
      ...
      in {
        packages = {
          Dhaos = Dhaos;
          default = withSystem system ({ config, ... }:
