...
;                                :run (fn [task]
;                                       (tset (. vim.bo task.strategy.bufnr) :filetype :OverseerTask)
;                                       (vim.api.nvim_win_set_buf 0 (task:get_bufnr))
;                                       (set vim.wo.statuscolumn "%s")
;                                       (util.scroll_to_end 0))
;                                 :condition (fn [task]
;                                              (let [bufnr (task:get_bufnr)]
;                                                (and bufnr (vim.api.nvim_buf_is_valid bufnr))))}
...
(setup :overseer {:templates [ "cargo" "just" "make" "npm" "shell" "tox" "vscode" "mix" "rake" "task" "user"]})
