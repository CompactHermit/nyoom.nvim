...
(fn qn_choose_line [opt]
  (match opt
      (where cond1 (= opt :Create)) (vim.ui.input {:prompt "Create a Line # :: "} (fn [choice]
                                                                                   (qn.NewNoteAtLine (tonumber choice))))
      (where cond2 (= opt :Open)) (vim.ui.input {:prompt "Open at Line #:: "} (fn [choice]
                                                                                (qn.OpenNoteAtLine (tonumber choice))))))

(nyoom-module-p! neorg
   (do
      (map! [n] :<leader>nqC `(qn.NewNoteAtCWD) {:desc "Create Note:: CWD"})
      (map! [n] :<leader>nql `(qn_choose_line :Create) {:desc "Create Note:: Line + Input"})
      ...
      (map! [n] :<leader>nqO `(qn_choose_line :Open) {:desc "Open Note:: Choose Line"})
      ...
      (map! [n] :<leader>nqs `(qn.ShowNoteSigns) {:desc "Show Signs"})
